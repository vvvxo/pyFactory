...
def layout(pro_series):
    ...
    for i in range(0, len(pro_series)-1):
        ...
        if front_x == 0:
            ...
            # 判断换行
            testMaxX = sF.area[numPro][0] + sF.coord[pro_series[i-1]
                                                     ][0] + sF.area[pro_series[i - 1]][0] / 2
            if (testMaxX > sF.fX):
                print("换行")
                sF.coord[numPro][0] = sF.area[numPro][0] / \
        ...
        if i == 0:
            ...
            for j in range(0, i - 1):
                ...
                if (testMax < testMin):  # 如果有交集
                    ...
                    if sF.coord[numPro][1] < sF.coord[pro_series[j-1]][1]+sF.area[pro_series[j-1]][1]/2+sF.area[numPro][1]+safe_distance:
                        sF.coord[numPro][1] = sF.coord[pro_series[j-1]][1] + \
                            sF.area[pro_series[j-1]][1]/2 + \
                            sF.area[numPro][1]+safe_distance
                else:
                    sF.coord[numPro][1] = sF.area[numPro][1] / \
                        2 + safe_distance
